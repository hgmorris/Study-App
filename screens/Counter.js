...
stepPoints.length=30;
  try{
    const tokenResponse = await fetch('https://dev.stedi.me/',{
  method: 'POST',
  body:JSON.stringify({
    userName: "hinnehmorris4@gmail.com",
...
await fetch('https://dev.stedi.me/rapidsteptest',{
  ...
  body:JSON.stringify({
customer:'hinnehmorris4@gmail.com',
...
try{
  const scoreResponse = await fetch('https://dev.stedi.me/riskscore/hinnehmorris4@gmail.com',{
  method:'GET',
  headers:{
    'Content-Type': 'application/json',
   'suresteps.session.token': token.current
